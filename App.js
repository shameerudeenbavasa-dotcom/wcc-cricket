...
// ─── THEME ────────────────────────────────────────────────────────────────────
const T = {
  gold: '#D4A017', green: '#1B5E20', dark: '#080C08', card: '#0E140E', 
  border: '#1E2E1E', text: '#F0EAD6', muted: '#7A8C6A', red: '#C62828',
};
  ...
  return (
    <SafeAreaView style={s.fullDark}>
      <View style={s.center}>
        ...
        <TouchableOpacity style={s.ghostBtn} onPress={() => onLogin('viewer')}>
...
function TeamMgmt({ onBack, onSave }) {
  ...
  const saveTeam = () => {
    ...
    Alert.alert('Success', 'Team saved to local state!');
    onBack();
  };
  ...
  return (
    <SafeAreaView style={s.fullDark}>
      <ScrollView style={{padding: 20}}>
        ...
        <TextInput style={s.input} onChangeText={setTName} placeholder="E.g. Weekend Cricket Club" placeholderTextColor={T.muted} />
        ...
        <TouchableOpacity style={s.greenBtn} onPress={() => {if(pName) {setPlayers([...players, {pName, jersey}]); setPName(''); setJersey('');}}}>
          <Text style={s.btnTxt}>+ ADD TO LIST</Text>
        </TouchableOpacity>
        {players.map((p, i) => <Text key={i} style={s.mutedTxt}>{p.jersey} - {p.pName}</Text>)}
        <TouchableOpacity style={s.goldBtn} onPress={saveTeam}><Text style={s.btnTxt}>SAVE TEAM ONLINE</Text></TouchableOpacity>
...
function AdminScreen({ onLogout, onManage, score, setScore }) {
  ...
  const toggleLive = async () => {
    if (isLive) { 
        ...
        try {
          ...
          engine.current = _eng;
          setIsLive(true);
        } catch (e) { Alert.alert('Error', 'Check Agora ID'); }
    }
  };
  ...
  return (
    <SafeAreaView style={s.fullDark}>
      ...
      <TouchableOpacity style={s.card} onPress={onManage}><Text style={s.btnTxt}>Manage Teams & Players</Text></TouchableOpacity>
...
function ViewerScreen({ onLogout, score }) {
    return (
        <SafeAreaView style={s.fullDark}>
            ...
            <View style={s.cameraBox}><Text style={s.mutedTxt}>Waiting for Admin Stream...</Text></View>
            <View style={s.card}><Text style={s.scoreText}>{score.runs} / {score.wickets}</Text></View>
            <TouchableOpacity onPress={onLogout}><Text style={s.centerMuted}>Exit to Login</Text></TouchableOpacity>
        </SafeAreaView>
    );
}
